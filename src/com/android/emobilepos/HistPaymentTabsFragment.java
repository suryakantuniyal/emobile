...
import com.android.histpayment.fragments.HistPaymentFragFour;
import com.android.histpayment.fragments.HistPaymentFragOne;
import com.android.histpayment.fragments.HistPaymentFragThree;
import com.android.histpayment.fragments.HistPaymentFragTwo;

public class HistPaymentTabsFragment extends Fragment implements OnTabChangeListener {
	private static final String[] TABS = new String[] { "cash", "check", "card","giftcard", "other" };
	private static String[] TABS_TAG;// = new String[] { "Cash", "Check", "Card", "Other" };
	private static final int[] TABS_ID = new int[] { R.id.cash_tab, R.id.check_tab, R.id.card_tab,R.id.giftcard_tab, R.id.other_tab };
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		TABS_TAG = new String[] {resources.getString(R.string.pay_tab_cash),resources.getString(R.string.pay_tab_check),
				resources.getString(R.string.pay_tab_card),resources.getString(R.string.pay_tab_giftcard),resources.getString(R.string.pay_tab_other)};
	...
	private void updateMyTabs(String tabID, int placeHolder) {
		...
		if (fragManager.findFragmentByTag(tabID) == null) {
			if (tabID.equals(TABS[0])) {
				fragManager.beginTransaction().replace(placeHolder, new HistPaymentFragOne(), tabID).commit();
			} else if (tabID.equals(TABS[1])) {
				fragManager.beginTransaction().replace(placeHolder, new HistPaymentFragTwo(), tabID).commit();
			} else if (tabID.equals(TABS[2])) {
				fragManager.beginTransaction().replace(placeHolder, new HistPaymentFragThree(), tabID).commit();
			} else if (tabID.equals(TABS[3])) {
				fragManager.beginTransaction().replace(placeHolder, new HistPaymentFragFour(), tabID).commit();
			} else {
				fragManager.beginTransaction().replace(placeHolder, new HistPaymentFragFour(), tabID).commit();
			}
		}
