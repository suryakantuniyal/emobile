...
import com.android.emobilepos.payment.ProcessCreditCard_FA.PAYMENT_GIFT_CARDS;
...
public class ProcessGiftCard_FA extends FragmentActivity implements EMSCallBack,OnClickListener {
	...
	private void setupHeaderTitle()
	{
		...
		if(custidkey==null)
			...
			fieldAmountDue.setText(Global.getCurrencyFormat(Global.formatNumToLocale(Double.parseDouble(extras.getString("amount")))));
			if (!isFromMainMenu) {
...
//			}
	...
	private void processPayment()
	{
		...
		String amountToBePaid = Double.toString(Global.formatNumFromLocale(fieldAmountPaid.getText().toString()
				.replaceAll("[^\\d\\,\\.]", "").trim()));
		...
		{
			payment.pay_type = "0";
			generatedURL = payGate.paymentWithAction("ChargeGiftCardAction", wasReadFromReader,cardType,cardInfoManager);
		...
		else
		{
			payment.is_refund = "1";
			payment.pay_type = "2";
			generatedURL = payGate.paymentWithAction("ReturnGiftCardAction", wasReadFromReader,cardType,cardInfoManager);
		}
