	...
	public static class PrefsFragment extends PreferenceFragment implements OnPreferenceClickListener
	{
		...
		public void onCreate(Bundle savedInstanceState) 
		{
			...
			case SettingsTab_FR.CASE_ADMIN:
				...
				prefManager.findPreference(MyPreferences.pref_config_genius_peripheral).setOnPreferenceClickListener(this);
				...
				{
					CharSequence c = new String("\t\t"+getString(R.string.admin_close_shift)+" <"+myPref.getShiftClerkName()+">");
				...
				CheckBoxPreference _cbp_use_location_inv = (CheckBoxPreference)prefManager.findPreference("pref_enable_location_inventory");
				
				
				storeForwardTransactions = (Preference) prefManager.findPreference("pref_store_and_forward_transactions");
				...
				if(!myPref.storedAndForward(true, false))
				{
					...
					((PreferenceGroup)prefManager.findPreference("payment_section")).removePreference(storeForwardFlag);
				}
				//((PreferenceGroup)prefManager.findPreference("payment_section")).removePreference(storeForwardTransactions);
		...
		{
			ListPreference lp = (ListPreference)getPreferenceManager().findPreference(MyPreferences.pref_default_category);
		...
		{
			ListPreference lp = (ListPreference)getPreferenceManager().findPreference(MyPreferences.pref_default_payment_method);
		...
		private void promptCloseShift(final boolean askAmount,final double amount)
		{
			...
			else
			{
				...
				viewTitle.setText(R.string.dlog_title_confirm);
				viewMsg.setText(activity.getString(R.string.close_amount_is)+" "+Global.formatDoubleToCurrency(amount));
...
//					{
//						if(Global.multiPrinterManager.get(i).currentDevice==null)//not connected
//						{
//							if (Global.multiPrinterManager.get(i).loadMultiDriver(activity, Global.STAR, 48, true,"TCP:"+ c.getString(c.getColumnIndex("printer_ip")),
//									c.getString(c.getColumnIndex("printer_port"))))
//								sb.append(c.getString(c.getColumnIndex("printer_ip"))).append(": ").append("Connected\n");
//							else
//								sb.append(c.getString(c.getColumnIndex("printer_ip"))).append(": ").append("Failed to connect\n");
//						}
//						else
//						{
//							sb.append(c.getString(c.getColumnIndex("printer_ip"))).append(": ").append("Connected\n");
