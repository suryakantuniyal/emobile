...
public class InvoicesHandler {
	...
	private static final List<String> attr = Arrays.asList(new String[] { inv_id, cust_id, emp_id, inv_timecreated, inv_ispending, inv_ponumber,
			inv_terms, inv_duedate, inv_shipdate, inv_shipmethod, inv_total, inv_apptotal, inv_balance, inv_custmsg, inv_ispaid, inv_paiddate,
			mod_date, txnID, inv_update });
...
//
//		StringBuilder sb = new StringBuilder();
//		sb.append("SELECT inv_id FROM ").append(table_name).append(" WHERE inv_id = (select max(inv_id) FROM ").append(table_name).append(")");
//
//		SQLiteStatement stmt = DBManager._db.compileStatement(sb.toString());
//
...
//	}
	...
	public Cursor getListSpecificInvoice(String id )
	{
		...
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT i.inv_id as _id, c.cust_name, i.txnID,i.inv_balance,i.inv_ispaid,i.inv_total,i.inv_duedate,i.inv_shipdate,i.inv_timecreated " +
	...
	public Cursor getSearchedInvoicesList(String pattern,boolean isForSpecificCust) {
		...
		//Cursor cursor = db.rawQuery(sb.toString(), new String[] {"%" + pattern + "%"});
	...
	public String[] getSpecificInvoice(String invID)
	{
		...
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT c.cust_name, i.inv_id , i.txnID,i.inv_total, i.inv_balance,i.inv_timecreated,i.inv_duedate,i.inv_shipdate,i.inv_ispaid " +
		...
		if(cursor.moveToFirst())
		{
			do
			{
				...
				arrayVal[5] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(inv_timecreated)),activity,0);
				arrayVal[6] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(inv_duedate)),activity,0);
				arrayVal[7] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(inv_shipdate)),activity,0);
