...
public class StoredPayments_DB {
	...
	public final List<String> attr = Arrays.asList(new String[] { pay_id, group_pay_id, cust_id,tupyx_user_id, emp_id, inv_id, paymethod_id, pay_check,
			...
			IvuLottoNumber,IvuLottoQR,card_type,Tax1_amount,Tax1_name,Tax2_amount,Tax2_name,custidkey,original_pay_id,pay_uuid,is_retry,payment_xml});
...
//	}
	...
	public String updateSignaturePayment(String _pay_uuid)
	{
		...
		sb.setLength(0);
		sb.append("SELECT pay_amount FROM ").append(table_name).append(" WHERE pay_uuid = '").append(_pay_uuid).append("'");
...
//			db = dbManager.openReadableDB();
	...
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT Count(*) FROM ").append(table_name).append(" WHERE job_id = '").append(_job_id).append("' AND is_retry = '1'");
