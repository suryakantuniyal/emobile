...
public class ConsignmentTransactionHandler 
{
	...
	private final List<String>attr = Arrays.asList(new String[]{Cons_ID,ConsTrans_ID,ConsEmp_ID,
			ConsCust_ID,ConsInvoice_ID,ConsReturn_ID,ConsPickup_ID,ConsDispatch_ID,ConsProd_ID,ConsOriginal_Qty,ConsStock_Qty,
	...
	public void insert(List<ConsignmentTransaction>list)
	{
		...
		for (int i = 0; i < size; i++) 
		{
			...
			//values.put(is_synched,list.get(i).getSetData(is_synched, true, null));
	...
	public List<ConsignmentTransaction>getLastConsTransaction()
	{
		...
		/*sb.append("SELECT * FROM CustomerInventory ci LEFT OUTER JOIN ConsignmentTransaction ct ON ci.prod_id = ct.ConsProd_ID  ");
		sb.append("AND ci.cust_id = ct.ConsCust_ID WHERE ct.ConsCust_ID = '");
		sb.append(myPref.getCustID()).append("'  GROUP BY ct.ConsProd_ID ORDER BY ct.Cons_timecreated DESC ");
		*/
		
		sb.append("SELECT ct.ConsProd_ID,ct.ConsOriginal_Qty,ct.ConsStock_Qty,ct.ConsReturn_Qty,ct.ConsInvoice_Qty,ct.ConsDispatch_Qty,");
		sb.append("ct.ConsNew_Qty, p.prod_price as 'master_price',vp.price as 'volume_price', ch.over_price_net as 'chain_price',");
		sb.append("pl.pricelevel_price FROM CustomerInventory ci LEFT OUTER JOIN ConsignmentTransaction ct ON ");
		sb.append("ci.prod_id = ct.ConsProd_ID  AND ci.cust_id = ct.ConsCust_ID LEFT OUTER JOIN Products p ON p.prod_id = ci.prod_id ");
		sb.append("LEFT OUTER JOIN VolumePrices vp ON ci.prod_id = vp.prod_id AND '1' BETWEEN vp.minQty AND vp.maxQty ");
		sb.append("AND vp.pricelevel_id = '").append(priceLevelID).append("' LEFT OUTER JOIN PriceLevelItems pl ON ");
		sb.append("ci.prod_id = pl.pricelevel_prod_id AND pl.pricelevel_id = '").append(priceLevelID).append("' LEFT OUTER JOIN ProductChainXRef ch ");
	...
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ").append(sb1.toString()).append(" FROM ").append(TABLE_NAME).append(" WHERE is_synched = '1'");
	...
	public Cursor getConsignmentCursor(boolean typePickup)
	{
		...
		sb.append("SELECT ConsTrans_ID as '_id', c.cust_name,ct.is_synched,Cons_timecreated FROM ConsignmentTransaction ct ");
		...
		{
			sb.append("WHERE (ConsInvoice_ID != '' OR ConsReturn_ID != '' OR ConsDispatch_ID != '' OR (ConsInvoice_ID = '' AND ConsReturn_ID = '' AND ConsDispatch_ID = '' AND ConsPickup_ID = '')) ");
		}
	...
	public HashMap<String,String> getConsignmentSummaryDetails(String _ConsTrans_ID,boolean isPickup)
	{
		...
		sb.append("sum((((ConsOriginal_Qty-ConsStock_Qty)*p.prod_price)-(ConsReturn_Qty*p.prod_price))) as 'total_grand_total', ");
		sb.append("cs.encoded_signature FROM ConsignmentTransaction ct LEFT OUTER JOIN Products p ON ct.ConsProd_ID = p.prod_id ");
		sb.append("LEFT OUTER JOIN ConsignmentSignatures cs ON cs.ConsTrans_ID = ct.ConsTrans_ID WHERE ct.ConsTrans_ID = ?");
	...
	public Cursor getConsignmentItemDetails(String _ConsTrans_ID) {
		...
		sb.append("SELECT ConsOriginal_Qty,ConsStock_Qty,ConsReturn_Qty,ConsInvoice_Qty,ConsDispatch_Qty,ConsNew_Qty,ConsPickup_Qty,ci.price as 'price', ");
		sb.append("((ConsOriginal_Qty-ConsStock_Qty)*ci.price) as 'item_subtotal', (ConsReturn_Qty*ci.price) as 'credit_memo', ");
		sb.append("(((ConsOriginal_Qty-ConsStock_Qty)*ci.price)-(ConsReturn_Qty*ci.price)) as 'item_total',pi.prod_img_name,p.prod_name,p.prod_desc ");
		sb.append("FROM ConsignmentTransaction ct  LEFT OUTER JOIN CustomerInventory ci ON ct.ConsProd_ID = ci.prod_id AND ct.ConsCust_ID = ");
		sb.append("ci.cust_id LEFT OUTER JOIN Products_Images pi ON ct.ConsProd_ID = pi.prod_id AND pi.type = 'I' LEFT OUTER JOIN Products p ON ");
