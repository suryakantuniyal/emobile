...
public class CustomersHandler {
	...
	private final List<String> attr = Arrays.asList(new String[] { cust_id, cust_id_ref, qb_sync, zone_id, CompanyName, Salutation, cust_name,
			cust_chain, cust_balance, cust_limit, cust_contact, cust_firstName, cust_middleName, cust_lastName, cust_phone, cust_email, cust_fax,
			cust_update, isactive, cust_ordertype, cust_taxable, cust_salestaxcode, pricelevel_id, cust_terms, cust_pwd, cust_securityquestion,
			cust_securityanswer, cust_points,custidkey,cust_id_numeric,cust_dob,AccountNumnber });
	...
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT cust_id,cust_name,cust_firstName,cust_lastName,CompanyName,cust_contact,cust_phone,cust_email,cust_dob FROM Customers WHERE qb_sync = '0'");
...
//			db = dbManager.openReadableDB();
	...
	public List<String> getCustDetails(String custID) {
		...
		//StringBuilder sb = new StringBuilder().append(subquery1).append(custID).append("'");
		StringBuilder custAddress = new StringBuilder();
		String [] billingAddress = new String[7];
		String [] shippingAddress = new String[7];
		
		
		/*String[] fields = new String[] { cust_name, cust_contact, cust_phone, cust_email, CompanyName, cust_balance, cust_limit,
				cust_taxable };
		String[] arguments = new String[] { custID };
		Cursor cursor = db.query(true, table_name, fields, "cust_id=?", arguments, null, null, null, null);*/
		...
		if (cursor.moveToFirst()) 
		{
			do {
				...
				custAddress.append(getCustAddr(billingAddress[0])).append(jump).append(getCustAddr(billingAddress[1])).append(jump);
				custAddress.append(getCustAddr(billingAddress[2])).append(jump).append(jump).append(getCustAddr(billingAddress[5])).append(jump);
				custAddress.append(getCustAddr(billingAddress[3])).append(tab).append(getCustAddr(billingAddress[4])).append(tab);
				custAddress.append(getCustAddr(billingAddress[6]));
				list.add(custAddress.toString().trim());
				
				custAddress.setLength(0);
				custAddress.append(getCustAddr(shippingAddress[0])).append(tab).append(getCustAddr(shippingAddress[1])).append(tab);
				custAddress.append(getCustAddr(shippingAddress[2])).append(tab).append(getCustAddr(shippingAddress[3])).append(tab);
				custAddress.append(getCustAddr(shippingAddress[4])).append(tab).append(getCustAddr(shippingAddress[6])).append(tab);
	...
	public Cursor getSearchCust(String search) // Transactions Receipts first listview
	{
//		if(db==null||!db.isOpen())
//			db = dbManager.openReadableDB();

		StringBuilder sb = new StringBuilder();
		sb.append("SELECT cust_id as _id,c.AccountNumnber, custidkey, cust_name,c.pricelevel_id,pl.pricelevel_name,cust_taxable,cust_salestaxcode,");
		sb.append("cust_email,CompanyName,cust_phone FROM Customers c LEFT OUTER JOIN PriceLevel pl ON c.pricelevel_id = pl.pricelevel_id ");
		sb.append("WHERE c.cust_name LIKE ? OR c.cust_id LIKE ? OR c.AccountNumnber LIKE ? OR c.cust_email LIKE ? OR c.cust_phone LIKE ? ORDER BY cust_name");
		
		Cursor cursor = DBManager._db.rawQuery(sb.toString(), new String[] {"%" + search + "%","%"+search+"%","%"+search+"%","%"+search+"%","%"+search+"%"});
		cursor.moveToFirst();
		return cursor;
	}
