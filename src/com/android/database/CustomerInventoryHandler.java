...
public class CustomerInventoryHandler 
{
	...
	private final List<String>attr = Arrays.asList(new String[]{cust_id,prod_id,
			qty,price,prod_name,cust_update});
	...
	public void insert(List<String[]>Data, List<HashMap<String,Integer>>dictionary)
	{
		...
		try {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("INSERT INTO ").append(TABLE_NAME).append(" (").append(sb1.toString()).append(",is_synched) ").append("VALUES (").append(sb2.toString())
					.append(",?)");
			...
			for(int i = 0 ; i < size; i++)
			{
				//tempMap = prodDBHandler.getProductMap(db, getData(prod_id,i));
				...
				//if(tempMap.get("prod_name")!=null)
					//insert.bindString(index(prod_name), tempMap.get("prod_name"));
	...
	public Cursor getCustomerInventoryCursor()
	{
		...
		/*sb.append("SELECT prod_id as '_id', prod_name,qty,price,cust_update,ROUND(qty*price,2) as 'total' FROM ").append(TABLE_NAME).append(" WHERE cust_id = '");
		sb.append(myPref.getCustID()).append("'");*/
		
		/*sb.append("SELECT ci.prod_id as '_id', p.prod_name,ci.qty,ci.price,ci.cust_update,ROUND(ci.qty*ci.price,2) as 'total' FROM CustomerInventory ");
		sb.append("ci LEFT OUTER JOIN Products p ON ci.prod_id = p.prod_id WHERE ci.cust_id = '").append(myPref.getCustID()).append("'");*/
		...
		sb.append("SELECT ci.prod_id as '_id', p.prod_name,ci.qty,ci.cust_update,ci.price as 'cust_price',p.prod_price as 'master_price',vp.price as 'volume_price', ch.over_price_net as 'chain_price',");
		sb.append("pl.pricelevel_price FROM CustomerInventory ci LEFT OUTER JOIN Products p ON ci.prod_id = p.prod_id LEFT OUTER JOIN VolumePrices vp ON ");
		...
		sb.append(" LEFT OUTER JOIN ProductChainXRef ch ON ci.prod_id = ch.prod_id AND ch.cust_chain = ci.cust_id WHERE ci.cust_id = ?");
		
		Cursor cursor = DBManager._db.rawQuery(sb.toString(), new String[]{priceLevelID,priceLevelID,myPref.getCustID()});
		cursor.moveToFirst();
		//db.close();
		return cursor;
	}
	...
	public void getCustomerInventory()
	{
		...
		StringBuilder sb = new StringBuilder();
		/*sb.append("SELECT * FROM ").append(TABLE_NAME).append(" WHERE cust_id = '");
		sb.append(myPref.getCustID()).append("'");*/
		
		/*sb.append("SELECT ci.prod_id, p.prod_name,ci.qty,ci.price FROM CustomerInventory ci LEFT OUTER JOIN Products p ON ci.prod_id = p.prod_id ");
		sb.append("WHERE cust_id = '").append(myPref.getCustID()).append("'");*/
		...
		sb.append("SELECT ci.prod_id, p.prod_name,ci.qty,p.prod_price as 'master_price',vp.price as 'volume_price', ch.over_price_net as 'chain_price'");
		sb.append(",pl.pricelevel_price FROM CustomerInventory ci LEFT OUTER JOIN Products p ON ci.prod_id = p.prod_id LEFT OUTER JOIN ");
		sb.append("VolumePrices vp ON ci.prod_id = vp.prod_id AND '1' BETWEEN vp.minQty AND vp.maxQty AND vp.pricelevel_id = ?");
		sb.append(" LEFT OUTER JOIN PriceLevelItems pl ON ci.prod_id = pl.pricelevel_prod_id AND pl.pricelevel_id = ?");
		sb.append(" LEFT OUTER JOIN ProductChainXRef ch ON ci.prod_id = ch.prod_id AND ch.cust_chain = ci.cust_id WHERE ci.cust_id = ? AND qty != '0'");
	...
	public double getProdQty(String prodID)
	{
		...
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ").append(qty).append(" FROM ").append(TABLE_NAME).append(" WHERE prod_id = ? AND cust_id = ?");
	...
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ").append(sb1.toString()).append(" FROM ").append(TABLE_NAME).append(" WHERE is_synched = '0'");
