...
public class ShiftPeriodsDBHandler 
{
	...
	public final List<String> attr = Arrays.asList(new String[] { shift_id, assignee_id, assignee_name, creationDate, 
			creationDateLocal, startTime, startTimeLocal, endTime, endTimeLocal, beginning_petty_cash, ending_petty_cash, entered_close_amount,
			total_transaction_cash,shift_issync});
	...
	public void insert(ShiftPeriods periods) {
		...
		try {
			...
			insert.bindString(index(creationDateLocal), periods.creationDateLocal==null?"":periods.creationDateLocal);	//creatingDateLocal
			...
			insert.bindString(index(beginning_petty_cash), periods.beginning_petty_cash==null?"0":periods.beginning_petty_cash);	//beginning_petty_cash
			insert.bindString(index(ending_petty_cash), periods.ending_petty_cash==null?"0":periods.ending_petty_cash);	//ending_petty_cash
			insert.bindString(index(entered_close_amount), periods.entered_close_amount==null?"0":periods.entered_close_amount);	//entered_close_amount
			insert.bindString(index(total_transaction_cash), periods.total_transaction_cash==null?"0":periods.total_transaction_cash);	//total_transaction_cash
...
//	}
	...
	public void updateShiftAmounts(String shiftID,double amount,boolean isReturn)
	{
		...
		sb.append("SELECT total_transaction_cash,ending_petty_cash FROM ").append(table_name).append(" WHERE shift_id=?");
	...
	public SparseArray<String>getShiftDetails(String shiftID)
	{
		...
		sb.append("SELECT assignee_name,beginning_petty_cash,ending_petty_cash,total_transaction_cash,ROUND(ending_petty_cash+total_transaction_cash,2) as 'total_ending_cash',entered_close_amount, CASE WHEN endTime != '' THEN endTime ELSE 'Open' END AS 'end_type' FROM ").append(table_name).append(" WHERE shift_id = ?");
	...
	public Cursor getAllShiftsReport(String date)
	{
		...
		sb.append("SELECT shift_id,startTime,date(creationDate,'localtime') as 'date', CASE WHEN endTime != '' THEN endTime ELSE 'Open' END AS 'end_type'," +
				"assignee_name,beginning_petty_cash FROM ").append(table_name).append(" WHERE date = ? ORDER BY startTime DESC");
	...
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT *,ROUND(ending_petty_cash+total_transaction_cash,2) as 'total_ending_cash' FROM ").append(table_name).append(" WHERE shift_issync = '0'");
	...
	public List<ShiftPeriods> getShiftDayReport(String clerk_id, String date)
	{
		...
		query.append("SELECT assignee_name,beginning_petty_cash,'0' as 'total_expenses',ending_petty_cash,total_transaction_cash,");
		query.append("ROUND(ending_petty_cash+total_transaction_cash,2) as 'total_ending_cash',entered_close_amount, startTime,");
		query.append("CASE WHEN endTime != '' THEN endTime ELSE 'Open' END AS 'end_type',date(startTime,'localtime') as 'date' FROM ShiftPeriods ");
