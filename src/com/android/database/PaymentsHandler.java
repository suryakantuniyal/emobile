...
public class PaymentsHandler {
	...
	public final List<String> attr = Arrays.asList(new String[] { pay_id, group_pay_id, cust_id,tupyx_user_id, emp_id, inv_id, paymethod_id, pay_check,
			pay_receipt, pay_amount,pay_dueamount, pay_comment, pay_timecreated, pay_timesync, account_id, processed, pay_issync, pay_transid, pay_refnum,
			pay_name, pay_addr, pay_poscode, pay_seccode, pay_maccount, pay_groupcode, pay_stamp, pay_resultcode, pay_resultmessage, pay_ccnum,
			pay_expmonth, pay_expyear, pay_expdate, pay_result, pay_date, recordnumber, pay_signature, authcode, status, job_id, user_ID, pay_type,
			pay_tip, ccnum_last4, pay_phone, pay_email, isVoid, pay_latitude, pay_longitude, tipAmount, clerk_id,is_refund,ref_num,IvuLottoDrawDate,
			IvuLottoNumber,IvuLottoQR,card_type,Tax1_amount,Tax1_name,Tax2_amount,Tax2_name,custidkey,original_pay_id});
	...
	public void insert(Payment payment) {
		...
		try {
			...
			insert.bindString(index(pay_resultmessage), payment.pay_resultmessage==null?"":payment.pay_resultmessage); // pay_resultmessage
			...
			insert.bindString(index(IvuLottoDrawDate), payment.IvuLottoDrawDate==null?"":payment.IvuLottoDrawDate);	//IvuLottoDrawData
...
//	}
	...
	public Cursor getUnsyncPayments() // Will return Cursor to all unsynchronized payments (used in generation of XML for post)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ").append(sb1.toString()).append(" FROM ").append(table_name).append(" WHERE pay_issync = '0'");
	...
	public String getTotalPayAmount(String paymethod_id,String pay_date)
	{
		...
		{
			sb.append("SELECT ROUND(SUM(pay_amount),2) AS 'total',date(pay_timecreated,'localtime') as 'date' FROM Payments ");
			sb.append("WHERE  paymethod_id = '' OR paymethod_id = '").append(paymethod_id);
		}
		else
			sb.append("SELECT ROUND(SUM(pay_amount),2) AS 'total',date(pay_timecreated,'localtime') as 'date' FROM Payments WHERE  paymethod_id = '").append(paymethod_id);
	...
	public String[] getPaymentDetails(String payID)
	{
		...
		sb.append("SELECT pay_date,pay_comment,job_id, inv_id,group_pay_id,pay_signature,ccnum_last4,pay_latitude,pay_longitude,isVoid,pay_transid,");
		...
		if(cursor.moveToFirst())
		{
			do
			{
				arrayVal[0] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(pay_date)),activity,0);
				arrayVal[1] = cursor.getString(cursor.getColumnIndex(pay_comment));
				arrayVal[2] = cursor.getString(cursor.getColumnIndex(inv_id));//is actually job_id as 'inv_id'
...
//	
	...
	public List<HashMap<String,String>> getPaymentDetailsForTransactions(String jobID)
	{
		...
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip',pm.paymentmethod_type AS 'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p,");
		sb.append("PayMethods pm WHERE p.paymethod_id = pm.paymethod_id AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','Wallet' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = 'Wallet' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','LoyaltyCard' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = 'LoyaltyCard' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','Reward' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = 'Reward' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','GiftCard' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = 'GiftCard' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','Genius' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = 'Genius' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.pay_tip AS 'pay_tip','Genius' AS  'paymethod_name',p.pay_id AS 'pay_id' FROM Payments p WHERE p.paymethod_id = '' ");
	...
	public Cursor getCashCheckGiftPayment(String type,boolean isRefund) {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM ")
				...
				.append(" p, PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE p.paymethod_id = m.paymethod_id " +
						"AND m.paymentmethod_type = '").append(type).append("' AND pay_type !='1' AND is_refund ='");
		if(isRefund)
	...
	public Cursor getCardPayments(boolean isRefund) {	
		...
		sb.append("'AmericanExpress' OR m.paymentmethod_type = 'Discover' OR m.paymentmethod_type = 'MasterCard' OR m.paymentmethod_type = 'Visa') AND pay_type !='1'  AND is_refund='");
	...
	public Cursor getOtherPayments(boolean isRefund) {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM Payments p, ");
		sb.append("PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE   p.paymethod_id = 'Wallet' ");
		...
		sb.append("m.paymentmethod_type != 'Cash' AND m.paymentmethod_type != 'GiftCard')) OR p.paymethod_id = 'Genius' OR p.paymethod_id = '' ");
	...
	public Cursor getLoyaltyPayments() {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM Payments p, ");
		sb.append("PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE p.paymethod_id = 'LoyaltyCard' ");
	...
	public Cursor getLoyaltyAddBalance() {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM Payments p, ");
		sb.append("PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE p.paymethod_id = 'LoyaltyCardBalance' ");
		sb.append("AND pay_type!=1 ");
	...
	public Cursor getGiftCardAddBalance() {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM Payments p, ");
		sb.append("PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE p.paymethod_id = 'GiftCardBalance' ");
		sb.append("AND pay_type!=1 ");
	...
	public Cursor getRewardAddBalance() {
		...
		sb.append("SELECT p.pay_id as _id,p.pay_amount,c.cust_name,p.job_id,p.isVoid,p.pay_issync,p.pay_tip FROM Payments p, ");
		sb.append("PayMethods m LEFT OUTER JOIN Customers c ON p.cust_id = c.cust_id WHERE p.paymethod_id = 'RewardBalance' ");
		sb.append("AND pay_type!=1 ");
...
//		sb.append("SELECT p.pay_amount,m.paymethod_name,p.pay_tip,p.pay_signature,p.pay_transid,p.ccnum_last4,p.IvuLottoDrawDate,p.IvuLottoNumber,p.IvuLottoQR FROM Payments p LEFT OUTER JOIN PayMethods m ON p.paymethod_id = m.paymethod_id " +
//				"WHERE job_id = '");
//		
//		sb.append(jobID).append("'");
		
		
		sb.append("SELECT p.pay_amount AS 'pay_amount',pm.paymethod_name AS 'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p,");
		sb.append("PayMethods pm WHERE p.paymethod_id = pm.paymethod_id AND p.job_id = '").append(jobID).append("' UNION ");
		
		sb.append("SELECT p.pay_amount AS 'pay_amount','Wallet' AS  'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p WHERE p.paymethod_id = 'Wallet' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		
		sb.append("SELECT p.pay_amount AS 'pay_amount','LoyaltyCard' AS  'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p WHERE p.paymethod_id = 'LoyaltyCard' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		
		sb.append("SELECT p.pay_amount AS 'pay_amount','Reward' AS  'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p WHERE p.paymethod_id = 'Reward' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		
		sb.append("SELECT p.pay_amount AS 'pay_amount','GiftCard' AS  'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p WHERE p.paymethod_id = 'GiftCard' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
		
		sb.append("SELECT p.pay_amount AS 'pay_amount',p.card_type AS  'paymethod_name',p.pay_tip AS 'pay_tip',p.pay_signature AS 'pay_signature',p.pay_transid AS 'pay_transid',p.ccnum_last4 AS 'ccnum_last4',p.IvuLottoDrawDate AS 'IvuLottoDrawDate',p.IvuLottoNumber AS 'IvuLottoNumber',p.IvuLottoQR AS 'IvuLottoQR',p.pay_dueamount AS 'pay_dueamount' FROM Payments p WHERE p.paymethod_id = 'Genius' ");
		sb.append("AND p.job_id = '").append(jobID).append("' UNION ");
	...
	public String[] getPrintingForPaymentDetails(String payID,int type)
	{
		...
		if(type == 0)						//May come from History>Payment>Details
		{
			...
			sb.append("SELECT p.inv_id,p.job_id,CASE WHEN p.paymethod_id IN ('Genius','') THEN p.card_type ELSE m.paymethod_name END AS 'paymethod_name',p.pay_date,p.pay_timecreated,IFNULL(c.cust_name,'Unknown') as 'cust_name', o.ord_total,p.pay_amount,p.pay_dueamount," +
		...
		{
			sb.append("SELECT p.inv_id,p.job_id,CASE WHEN p.paymethod_id IN ('Genius','') THEN p.card_type ELSE m.paymethod_name END AS 'paymethod_name',p.pay_date,p.pay_timecreated, IFNULL(c.cust_name,'Unknown') as 'cust_name',p.pay_amount AS 'ord_total',p.pay_amount,p.pay_dueamount," +
		...
		if(cursor.moveToFirst())
		{
			...
			do{
				...
				arrayVal[1] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(pay_date)),activity,0);
				arrayVal[2] = Global.formatToDisplayDate(cursor.getString(cursor.getColumnIndex(pay_timecreated)),activity,2);
	...
	public HashMap<String,String> getPaymentsRefundsForReportPrinting(String date, int type)
	{
		...
		case 0://search for Payments Summary
			sb.append("SELECT p.paymethod_id,p.pay_amount as 'total',date(pay_timecreated,'localtime') as 'date' FROM Payments p " +
		...
		if(cursor.moveToFirst())
		{
			do
			{
				if(map.containsKey(cursor.getString(cursor.getColumnIndex("paymethod_id"))))
				{
					...
					bg = bg.add(new BigDecimal(cursor.getString(cursor.getColumnIndex("total")))).setScale(2, RoundingMode.HALF_UP);
					map.put(cursor.getString(cursor.getColumnIndex("paymethod_id")), bg.toString());
				}
				else
					map.put(cursor.getString(cursor.getColumnIndex("paymethod_id")), cursor.getString(cursor.getColumnIndex("total")));
	...
	public List<Payment> getPaymentsDayReport(int type, String clerk_id, String date)
	{
		...
		query.append("SELECT card_type, pay_amount, pay_tip, pay_id, CASE WHEN inv_id ='' THEN job_ID ELSE inv_id END AS 'job_id',date(pay_timecreated,'localtime') as 'date' FROM Payments WHERE ");
	...
	public List<Payment> getPaymentsGroupDayReport(int type, String clerk_id, String date)
	{
		...
		query.append("SELECT card_type, SUM(pay_amount) AS 'pay_amount', SUM(pay_tip) AS 'pay_tip', pay_id, paymethod_id,");
		query.append("CASE WHEN inv_id ='' THEN job_ID ELSE inv_id END AS 'job_id',date(pay_timecreated,'localtime') as 'date' FROM Payments WHERE ");
