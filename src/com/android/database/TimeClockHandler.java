...

{
	...
	public final List<String> attr = Arrays.asList(new String[] { timeclockid,emp_id,status,punchtime,updated,issync});
	...
	public void insert(List<TimeClock>timeClock,boolean deleteAll)
	{
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				if(deleteAll)
					DBManager._db.delete(table_name, "emp_id = ? AND status = ?", new String[]{timeClock.get(i).emp_id,timeClock.get(i).status});
	...
	public Cursor getAllUnsync() // Will populate all unsynchronized orders for XML post
	{
//		if(db==null||!db.isOpen())
//			db = dbManager.openReadableDB();
		
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ").append(sb1.toString()).append(" FROM ").append(table_name).append(" WHERE issync = '0' and emp_id NOT NULL and trim(emp_id) <> ''");
