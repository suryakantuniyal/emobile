...
public class Global extends Application {
	...
	//public static List<Orders>consignment_cur_order = new ArrayList<Orders>();
	...
	//public static List<Orders>cons_fillup_cur_order = new ArrayList<Orders>();
	...
	//public static List<Orders>cons_return_cur_order = new ArrayList<Orders>();
	...
	//public static final Map<String, String> transactionType = createTransactionsMap();
...
//	}
	...
	public static String formatToDisplayDate(String date,Activity activity,int type)
	{
		...
		try {
			...
			sb.append(e.getMessage()).append(" [").append("com.android.support.Global (at Class.formatToDisplayDate) ]");
...
//		    return false;
	  ...
	  public static String formatLocaleToCurrency(String value)
	  {
		...
		public static String getCurrencyFormat(String value)//received as #
		{
			...
			/*else if(value.contains("."))
				return NumberFormat.getCurrencyInstance(Locale.US).format(Double.parseDouble(value));
			return NumberFormat.getCurrencyInstance(Locale.US).format((double)Integer.parseInt(value));*/
	  ...
	  public static double formatNumFromLocale(String val)//received as #,## instead of #.##
	  {
		  double frmt = 0.0;
		  try
		  {
			  ...
			  //Number number = NumberFormat.getNumberInstance(Locale.getDefault()).setParseIntegerOnly(false).parse(val);
	...
	public void refreshParticularOrder(MyPreferences myPref,int position,String[]data)
	{
		...
		String prLevTotal = data[2];//orderedProducts.getSetData("overwrite_price", true, null);
	...
	public static CreditCardInfo parseSimpleMSR(Activity activity, String data) {
		...
		if (tracks.length >= 2) // two tracks
		{
			if (!tracks[0].isEmpty()) {
				...
				if(tracks[1]==null||tracks[1].isEmpty())//track 2 N/A retrieve PAN from track 1
				{
			...
			if (tracks[1]!=null&&!tracks[1].isEmpty()) 
			{
				...
				card_number = track2Split[0].replace("?", "");// contains card number
...
//		}
	...
	public static boolean isConnectedToInternet(Activity activity)
	{
		...
		if((myWifi!=null&&myWifi.isAvailable()&&myWifi.isConnected())||(myMobile!=null&&myMobile.isAvailable()&&myMobile.isConnected())||
...
//				Process p = Runtime.getRuntime().exec("/system/bin/ping -c 1 www.google.com");
//				Thread.sleep(1000);
//				int i = p.exitValue();
//				if(i==0)
//					return true;
...
//	{
//		double prodPrice = getDouble(orderProducts.get(position).getSetData("overwrite_price", true, null));
//		double qty = getDouble(orderProducts.get(position).getSetData("ordprod_qty", true, null));
//		
//		double granTotal = prodPrice*qty;
//		orderProducts.get(position).getSetData("itemTotal", false, Double.toString(granTotal));
//	}
...
//		sb.append(sym.getDecimalSeparator()).append("\\d{2})?$");
		...
		if (!s.toString().matches(sb.toString())||!s.toString().contains(Character.toString(sym.getDecimalSeparator()))) {
	...
	{
		return (_printer_type==Global.STAR||_printer_type==Global.BAMBOO||_printer_type==Global.ZEBRA||_printer_type==Global.ASURA);
	}
	
	public static boolean deviceHasBarcodeScanner(int _device_type)
	{
		return (_device_type==Global.POWA||_device_type==Global.ASURA || _device_type==Global.STAR);
	}
