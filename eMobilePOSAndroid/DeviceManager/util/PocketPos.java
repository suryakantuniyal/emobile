 ...
 * Contributors:
 * 
 * Modifications: name version reasons
 * 
 */
...
public class PocketPos {
	...
	public final static byte FONT_SMALL = 1;// 小字体
	public final static byte FONT_BIG = (byte) 0xC0; // 0x08 = Double-height
														// mode selected 0x10 =
														// Double-width mode
														// selected
	...
	/*
	 * Bit Off/On Value Function 0 Select 32 dot font (24 char/per line) eg. 1B
	 * 21 00 0 -- 1 Select 24 dot font (36 char/per line) eg. 1B 21 01 1 -- --
	 * undefined 2 -- -- undefined 3 -- -- undefined off 0 Double-height mode
	 * not selected 4 on 1 Double-height mode selected off 0 Double-width mode
	 * not selected 5 on 1 Double-width mode selected 6 -- -- undefined off 0
	 * Underline mode not selected 7 on 1 Underline mode selected
	 */
	...
	private static String bytes2charsUTF8(byte[] buf, int offset, int length, boolean bigEndian) {
		...
		while (pos < (offset + length)) {
			if ((buf[pos] & 0x80) == 0x00) { // U-00000000 - U-0000007F:
												// 0xxxxxxx
				b1 = 0;
				b2 = buf[pos];
				pos++;
			} else if ((buf[pos] & 0xe0) == 0xc0) { // U-00000080 - U-000007FF:
													// 110xxxxx 10xxxxxx
	...
	public static byte[] FramePack(byte typeFrame, byte[] srcBuff, int srcOffset, int srcLen) {
		...
		if (typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP || typeFrame == FRAME_TOF_PRINT || typeFrame == FRAME_TOF_PWD
			...
			// byte[] buffer = new byte[(1+1+1) + 1 + 1 + BYTES_OF_DATA_LENGTH +
			// countSpecialBytes + (1+1+1)];
			byte[] buffer = new byte[(1) + 1 + 1 + BYTES_OF_DATA_LENGTH + countSpecialBytes + (1)]; // For
																									// P25
			// |C0H(SOF) | (TOF) | DATA_ID | DATA Length | DATA | CHECK_SUM
			// |C1H(EOF)|
			int offset = 0;
			buffer[offset] = B_SOF;
			offset++;
	...
	public static byte[] FrameUnpack(byte[] srcBuff, int srcOffset, int srcLen) {
		...
		if (typeFrame == FRAME_ETX && (offset + 2) <= length // 2 = 1+ 1 =
																// DATA_ID +
																// C1H(EOF)
		...
		if ((typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP || typeFrame == FRAME_MSR || typeFrame == FRAME_TOF_PWD || typeFrame == FRAME_TOF_PROCESSDATA)
				&& (offset + 4 + BYTES_OF_DATA_LENGTH) <= length) {// 4 = 1+2+1
																	// =
	...
	/*
	 * The tag field (T) consists of one or more consecutive bytes. It indicates
	 * a class, a type, and a number (see Table 35). The tag field of the data
	 * objects described in this specification is coded on one or two bytes.The
	 * length field (L) consists of one or more consecutive bytes. It indicates
	 * the length of the following field. The length field of the data objects
	 * described in this specification is coded on one or two bytes.The value
	 * field (V) indicates the value of the data object. If L = '00', the value
	...
	public static short[] TLVParse(byte[] data, int offset) {
		if (data != null && offset >= 0 && data.length > (offset + 2)) {
			...
			// rules of the subsequent bytes of a BER-TLV tag when tag numbers
			// ≥ 31
			...
			// 5个bit为“11111”，则说明该tag占两个字节，例如“9F33”；
			// 否则占一个字节，例如“95”。
			if ((data[i] & 0x1F) == 0x1F) {
			...
			i++;
			// When bit b8 of the most significant byte of the length field is
			// set to 0, the length
			// field consists of only one byte. Bits b7 to b1 code the number of
			// bytes of the value
			// field. The length field is within the range 1 to 127.
			// When bit b8 of the most significant byte of the length field is
			// set to 1, the
			// subsequent bits b7 to b1 of the most significant byte code the
			// number of
			// subsequent bytes in the length field. The subsequent bytes code
			// an integer
			// representing the number of bytes in the value field. Two bytes
			// are necessary to
